...
const inputs = [ //para uso local
  '2',
  '2',
  'OSADaaaOISAJDSAOIDJA',
  'ASOIJDOnnnSAJDASOIDJA',
  '2',
  'CBA',
  'DDD'
] //para uso local
let index = 1 //para uso local

const regexLine = /[A-Z]/g

// let nCase = parseInt(gets()); //para uso na DIO
let nCase = parseInt(inputs[0]); //para uso local
let L = 0;
let value;
let lines;
...
while (nCase > 0) {
  ...
  lines = []
  value = 0
  // L = parseInt(gets()) //para uso na DIO
  L = parseInt(inputs[index++]) //para uso local

  if (L < 1 || L > 100) {
    index += L; //para uso local
    for (let i = 0; i < L; i++) { gets(); } //para uso na DIO
    continue;
  }

  for (let i = 0; i < L; i++) {
    // lines = [...lines, gets()] //para uso na DIO
    lines = [...lines, inputs[i + index]] //para uso local
  }
