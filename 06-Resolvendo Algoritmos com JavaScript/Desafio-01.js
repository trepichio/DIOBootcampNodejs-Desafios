...
//Código do desafio
// const inputs = require('./Desafio-01-input01.js'); //para uso local
// const inputs = require('./Desafio-01-input02.js'); //para uso local
// const inputs = require('./Desafio-01-input03.js'); //para uso local
...
while (true) {
  ...
  n = parseInt(inputs[index]); //para uso local
  ...
  for (let i = 0; i < n; i++) {
    // [cards[0][i], cards[1][i], cards[2][i]] = gets().match(new RegExp(/\d+/, 'g')).map(v => parseInt(v)); //para uso na DIO
    [cards[0][i], cards[1][i], cards[2][i]] = inputs[index + i + 1].match(new RegExp(/\d+/, 'g')).map(v => parseInt(v)); //para uso local
  }

  console.log(canWinWith(0, 0, 0) ? 1 : 0);

  index += n + 1; //para uso local
}
